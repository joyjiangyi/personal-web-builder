...
**核心要求：**
- 一镜到底，无剪切、无转场、无闪白、无场景切换
- 运镜必须连续平滑，像摄影机沿一条完整轨道移动
- 视频是背景，不是主角——场景有深度感和运动感，但不抢走文字焦点
- 时长 **8 秒**（Kling 标准），16:9 横屏，高清

**提示词结构（时间戳分段式，必须用这套格式）：**

> 分段式提示词比单句英文提示词有效 10 倍。AI 需要明确的时间节奏指令，否则会自行剪辑。

```
生成一段 8 秒原生 16:9 高清电影感视频。

严格参考 [风格关键词]：[美术风格描述，色调，光线，质感]。

完整视频为一个连续的柔和子弹时间镜头，一镜到底。不要有任何切镜、转场、闪白、黑屏、淡入淡出、镜头重置或场景切换。

0-2 秒：
镜头从 [起始位置] 缓慢向 [目标方向] 推进，[场景元素] 在 [光线描述] 中自然存在。
[漂浮道具清单] 缓慢向前漂浮，节奏克制。

2-6 秒：
时间逐渐冻结，进入柔和子弹时间。[道具] 悬停在空气中，保持自然纵深。
镜头继续连续运动，以 [光源] 为中心，缓慢向 [方向] 移动，并做一次约 120 度的弧形环绕。
镜头从物件之间平稳穿过，近景 [道具A] 轻微掠过镜头边缘，中景 [道具B] 清晰可见，背景 [场景] 保持稳定。
物件只允许极轻微摆动，不要飞散，不要突然出现或消失。

6-8 秒：
子弹时间逐渐解除。[细微元素] 重新缓慢运动，[道具] 恢复轻柔漂浮。
镜头完成环绕后平稳停留，保持安静的 [色调] 光影，自然收尾。

运镜要求：
摄像机轨迹必须连续、平滑、稳定。前推与环绕必须自然衔接，像摄影机沿一条完整轨道移动。不要快速变焦，不要突然改变视角，不要晃动。画面保留足够留白，适合作为个人网站背景视频。

视觉要求：
高清、干净、层次清晰、低照度但细节可见、[主光源] 投影、轻微空气颗粒、克制的胶片颗粒、真实景深、[质感关键词]。

严格禁止：
任何镜头切换、场景变化、突然新增物件、密集杂物、人物、清晰文字、网址、水印、地图、芯片、电路板、AI界面、商业卡片、PPT感、蓝色科技风、赛博朋克、爆炸、动作大片式高速旋转、过曝。
```

**按用户身份填写场景 + 漂浮道具（对应用户的「核心张力」）：**

| 用户身份/张力 | 场景 | 漂浮道具（5-7 件，有叙事性） |
|-------------|------|-------------------------|
| 影视/创意人 | 空置摄影棚，单束戏剧顶光 | 场记板、剧本页、35mm 胶片、导演椅标牌、镜头盖、胶片票根 |
| AI/工程师 | 深空抽象光流 | 代码字符片段、电路节点光点、数据流丝带、算法图形轮廓 |
| 设计师/艺术家 | 极简白色工作室，漫射侧光 | 草图纸页、色彩卡、铅笔、颜料滴、胶带条、网格纸 |
| 摄影师 | 空旷风景，黄金时刻侧逆光 | 底片条、相机快门帘、感光纸、测光单、镜头滤镜 |
| 运营/营销 | 复古办公室，暖金台灯光 | 数据图表碎片、便利贴、报告封页、咖啡杯圈印、日历页 |
| 海洋/自然 | 深海蓝，水下折射光柱 | 气泡、光斑碎片、海草纤维、盐晶颗粒 |
| 奢华/高端 | 深色大理石，单束追光 | 烫金名片角、丝绒布料边、蜡印章、钢笔尖、信封边角 |
| 千禧/Y2K | 紫蓝渐变背景，全息反射 | 早期 CD、像素字符、MP3 播放器界面碎片、铬合金反光片 |

**输出格式：**

```
🎬 一镜到底视频提示词（推荐：Kling 图生视频 / Runway Gen-3）

[按上方模板填写的完整中文提示词]

---
推荐工具：
- Kling（国内，质量最稳）→ 图生视频模式，上传一张参考图效果更好
- Runway Gen-3 → 英文提示词，把上方提示词翻译后使用
- Pika Labs → 备选
...
- 如果生成的是 .mov，先转码：
  ffmpeg -i input.mov -c:v libx264 -crf 22 -movflags +faststart -pix_fmt yuv420p hero.mp4
- 命名为 hero.mp4，放到 index.html 同一文件夹，刷新即生效，无需改代码

没有条件生成视频？Pexels 备选：搜索 "[2-3个场景关键词] slow motion cinematic"，筛选 10s+、1080p、横屏
```
